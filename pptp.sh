...
echo "localip 192.168.1.111" >> /etc/pptpd.conf
echo "remoteip 192.168.1.111-133" >> /etc/pptpd.conf
...
iptables -t nat -A POSTROUTING -s 192.168.1.1/24 -j SNAT --to-source `ifconfig  | grep 'inet addr:'| grep -v '127.0.0.1' | cut -d: -f2 | awk 'NR==1 { print $1}'`
iptables -A FORWARD -p tcp --syn -s 192.168.1.1/24 -j TCPMSS --set-mss 1356
